...
template <class T, class = void> struct is_ifc4_or_higher : std::false_type {};

template <class T>
struct is_ifc4_or_higher<T, std::void_t<decltype(T::IfcMaterialDefinition)>>
    : std::true_type {};
...
std::string format_string(const Argument *argument) {
  // Argument is a runtime tagged variant for the various data types in a IFC
  // model, in this particular case we only care about flattening it to a
  // string.
...
void process_pset(element_properties &props, const T *inst) {
  ...
  if (auto pset = inst->template as<typename Schema::IfcPropertySet>()) {
    ...
    for (auto it = ps->begin(); it != ps->end(); ++it) {
      auto &p = *it;
      if (auto singleval =
              p->template as<typename Schema::IfcPropertySingleValue>()) {
        ...
        if (!singleval->NominalValue()) {
          ...
          props[*pset->Name()][propname] =
              format_string(singleval->NominalValue()
                                ->template as<IfcUtil::IfcBaseClass>()
                                ->data()
                                .getArgument(0));
        }
      }
  ...
  if (auto qset = inst->template as<typename Schema::IfcElementQuantity>()) {
    ...
    for (auto it = qs->begin(); it != qs->end(); ++it) {
      auto &q = *it;
      if (q->template as<typename Schema::IfcPhysicalSimpleQuantity>() &&
          q->data().getArgument(3)->type() == IfcUtil::Argument_DOUBLE) {
  ...
  if constexpr (is_ifc4_or_higher<Schema>::value) {
    if (auto extprops =
            inst->template as<typename Schema::IfcExtendedProperties>()) {
      // @todo
    }
  }
}
...
void get_psets_s(element_properties &props,
                 const typename Schema::IfcObjectDefinition *inst) {
  ...
  if constexpr (is_ifc4_or_higher<Schema>::value) {
    if (auto mdef =
            inst->template as<typename Schema::IfcMaterialDefinition>()) {
  ...
  if (auto ob = inst->template as<typename Schema::IfcObject>()) {
    ...
    }
    {
      ...
      for (auto it = rels->begin(); it != rels->end(); ++it) {
        auto &rel = *it;
        if (auto bytype =
                rel->template as<typename Schema::IfcRelDefinesByType>()) {
          get_psets_s<Schema>(props, bytype->RelatingType());
        } else if (auto byprops =
                       rel->template as<
                           typename Schema::IfcRelDefinesByProperties>()) {
...
// What follows is machinery to create a preprocessor-based dispatch mechanism
// to dispatch to the correct get_psets_s<Schema>() based on
// inst->declaration().schema()->name().
...
#define TEST_AND_DISPATCH(r, data, elem)                                       \
  if (strcasecmp(schema_name, GENERATE_LITERAL_STRING(elem)) == 0) {           \
    get_psets_s<EXPAND_AND_CONCATENATE(elem)>(                                 \
        props, inst->as<EXPAND_AND_CONCATENATE(elem)::IfcObjectDefinition>()); \
  }
...
int main(int argc, char **argv) {
  ...
  IfcSchema::IfcBuildingElement::list::ptr elements =
      file.instances_by_type<IfcSchema::IfcBuildingElement>();

  std::cout << "Found " << elements->size() << " elements in " << argv[1] << ":"
            << std::endl;
  ...
  for (auto it = elements->begin(); it != elements->end(); ++it) {
    ...
    for (auto &ps : props) {
      ...
      for (auto &p : ps.second) {
        std::cout << p.first << std::string(max_key_len - p.first.size(), ' ')
                  << ":" << p.second << std::endl;
      }
      std::cout << std::endl;
    }
